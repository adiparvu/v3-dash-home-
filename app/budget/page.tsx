      ...
      <div className="px-4 space-y-5">
        ...
        <div className="rounded-3xl p-4 liquid-glass">
          <div className="flex items-end justify-between mb-2">
            ...
            <p className="text-text-secondary text-xs">of €{MONTHLY_BUDGET} budget</p>
          </div>
          ...
          <p className="text-[11px] mt-1.5" style={{ color: status.over ? "#EF4444" : "var(--text-2)" }}>
            {status.over ? `€${Math.abs(status.remaining).toFixed(0)} over budget` : `€${status.remaining.toFixed(0)} remaining · ${status.pct}% used`}
          </p>
        </div>

        {/* Monthly trend */}
        <div>
          <p className="text-text-secondary text-xs font-medium uppercase tracking-wide mb-2 px-1">Monthly trend</p>
          <div className="rounded-2xl p-4 liquid-glass">
            ...
            <p className="text-text-tertiary text-[10px] mt-2 text-center">Total tracked: €{summary.total.toFixed(0)}</p>
          </div>
        </div>

        {/* By category */}
        <div>
          <p className="text-text-secondary text-xs font-medium uppercase tracking-wide mb-2 px-1">By category</p>
              ...
              return (
                <div key={c.category} className="px-4 py-3" style={{ borderBottom: i < summary.byCategory.length - 1 ? "1px solid rgba(255,255,255,0.06)" : undefined }}>
                  <div className="flex items-center gap-3 mb-1.5">
                    ...
                    <span className="flex-1 text-sm font-medium" style={{ color: "var(--text-1)" }}>{c.category}</span>
        ...
        {/* Recent expenses */}
        <div>
          <p className="text-text-secondary text-xs font-medium uppercase tracking-wide mb-2 px-1">Recent expenses</p>
          <div className="rounded-2xl liquid-glass overflow-hidden">
            {recent.map((e, i) => {
              ...
              return (
                <div key={e.id} className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: i < recent.length - 1 ? "1px solid rgba(255,255,255,0.06)" : undefined }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-text-secondary text-[11px]">{e.zone} · {e.date} · {e.source === "receipt" ? "🧾 receipt" : e.source === "bank" ? "🏦 bank" : "manual"}</p>
                  </div>
          ...
          <p className="text-text-tertiary text-[11px] text-center mt-2">Imported from Receipt Scanner & banking integrations.</p>
        </div>
      </div>
