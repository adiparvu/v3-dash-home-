...
type Option = { id: string; labelKey: MessageKey; icon: string; href: string; color: string };
...
function buildOptions(contact: CallContact): Option[] {
  ...
  if (phone) {
    opts.push({ id: "phone", labelKey: "call.phone", icon: "📞", href: `tel:${phone}`, color: "#4ADE80" });
    opts.push({ id: "ft-audio", labelKey: "call.ftAudio", icon: "🎧", href: `facetime-audio:${phone}`, color: "#22D3EE" });
    opts.push({ id: "ft-video", labelKey: "call.ftVideo", icon: "📹", href: `facetime:${phone}`, color: "#22D3EE" });
    opts.push({ id: "wa-audio", labelKey: "call.waAudio", icon: "🔊", href: `https://wa.me/${waNumber}`, color: "#25D366" });
    opts.push({ id: "wa-video", labelKey: "call.waVideo", icon: "🎥", href: `https://wa.me/${waNumber}`, color: "#25D366" });
  }

  const tg = contact.telegram?.replace(/^@/, "");
  if (tg) {
    opts.push({ id: "tg-audio", labelKey: "call.tgAudio", icon: "🔊", href: `https://t.me/${tg}`, color: "#229ED9" });
    opts.push({ id: "tg-video", labelKey: "call.tgVideo", icon: "🎥", href: `https://t.me/${tg}`, color: "#229ED9" });
  } else if (phone) {
    opts.push({ id: "tg-audio", labelKey: "call.tgAudio", icon: "🔊", href: `https://t.me/${waNumber}`, color: "#229ED9" });
    opts.push({ id: "tg-video", labelKey: "call.tgVideo", icon: "🎥", href: `https://t.me/${waNumber}`, color: "#229ED9" });
  }
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center" style={{ background: "rgba(0,0,0,0.45)" }} onClick={onClose}>
      <div className="w-full md:w-[390px] rounded-t-[28px] p-5 pb-8 animate-slide-up liquid-glass-strong" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="font-bold text-lg mb-0.5" style={{ color: "var(--text-1)" }}>{t("call.title")} {contact.name}</h2>
        <p className="text-sm mb-4" style={{ color: "var(--text-2)" }}>{t("call.choose")}</p>

        {options.length === 0 ? (
          <p className="text-text-secondary text-sm py-6 text-center">{t("call.noPhone")}</p>
        ) : (
          <div className="rounded-2xl overflow-hidden mb-3" style={{ background: "var(--glass-bg)", border: "0.5px solid var(--glass-border)" }}>
            {options.map((o, i) => (
              ...
              >
                ...
                <span className="text-sm font-medium flex-1" style={{ color: "var(--text-1)" }}>{t(o.labelKey)}</span>
        ...
        <button onClick={onClose} className="w-full py-3.5 rounded-2xl font-medium text-base" style={{ background: "var(--glass-bg)", border: "0.5px solid var(--glass-border)", color: "var(--text-1)" }}>{t("call.cancel")}</button>
      </div>
    </div>
  );
}
